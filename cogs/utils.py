...
#   ╚══════╝╚═╝░░╚══╝░╚═════╝░╚══════╝╚═╝╚═════╝░╚═╝░░╚═╝ 
    ...
    async def __random(self, ctx, count = None):
        if count == None:
            ...
            emb.set_footer(text = f'Copyright © 2020 {bot_name} | All rights reserved.', icon_url = self.client.user.avatar_url)
            await ctx.send(embed = emb)
            ...
            try:
                ...
                msg = await ctx.send(embed = discord.Embed(description = 'Mistake! Third party symbols were found!', color = cogs_color['RANDOM COLOR ERROR']))
                emb.set_footer(text = f'Copyright © 2020 {bot_name} | All rights reserved.', icon_url = self.client.user.avatar_url)
    ...
    async def __wiki(self, ctx, *, text = None):
        if text == None:
            ...
            try:
                ...
                emb.set_footer(icon_url = self.client.user.avatar_url, text = f"Copyright © 2020 {bot_name} | All rights reserved.")
                await ctx.send(embed = emb)
                ...
                emb.set_footer(text = f'Copyright © 2020 {bot_name} | All rights reserved.', icon_url = self.client.user.avatar_url)
                await ctx.send(embed = emb)
                print(f'[Logs:error] Произошла неизвестная ошибка! | {prefix}wiki [EU]')
...
#╚═╝░░╚═╝░╚═════╝░╚═════╝░╚═════╝░╚═╝╚═╝░░╚═╝╚═╝░░╚══╝               
    ...
    async def ___random(self, ctx, count = None):
        if count == None:
            ...
            emb.set_footer(text = f'Copyright © 2020 {bot_name} | Все права защищены.', icon_url = self.client.user.avatar_url)
            await ctx.send(embed = emb)
    ...
    async def ___wiki(self, ctx, *, text = None):
        if text == None:
            ...
            try:
                ...
                emb.set_footer(icon_url = self.client.user.avatar_url, text = f"Copyright © 2020 {bot_name} | Все права защищены.")
                await ctx.send(embed = emb)
                print(f'[Logs:utils] Информация о "{text}" была выведена | {prefix}wiki [RU]')
            except:
                emb = discord.Embed(description = f'Произошла неизвестная ошибка!', color = cogs_color['WIKIPEDIA COLOR ERROR'])   
                emb.set_footer(text = f'Copyright © 2020 {bot_name} | Все права защищены.', icon_url = self.client.user.avatar_url)
                await ctx.send(embed = emb)
                print(f'[Logs:error] Произошла неизвестная ошибка! | {prefix}wiki [EU]')            
