...
class Del_emoji(commands.Cog):
    """Remove specific user emoji from message"""
    ...
    async def __delemoji(self, ctx, id: int = None, reaction: str = None, member: discord.Member = None):
        if id != None and reaction != None:
            ...
            emb = discord.Embed(description = f'Example: `{prefix}del_emoji <message id> <emoji> [@user]` - Remove specific user emoji from message.', color = cogs_color['DELETE EMOJI COLOR EXAMPLE'])
