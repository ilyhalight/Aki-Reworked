...
from useful import fucku_en, prefix
...
class Fuck(commands.Cog):
    ...
    async def __fuckyou(self, ctx, user: discord.Member = None):
        ...
        if not user:
            ...
            fuck = randchoice(useful.fucku_en).format(user.mention, auth.mention)
            emb = discord.Embed(color = cogs_color['FUCK U'])
            emb.add_field(name = "Fuck You!:middle_finger:", value = "{}".format(fuck))
            await ctx.send(embed = emb)
def setup(bot):
